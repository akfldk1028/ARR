"""
HANG 노드에 OpenAI 임베딩 추가 (Django 없이 실행)

OpenAI text-embedding-3-large 모델 사용 (3072-dim)
"""
...
# .env file is at backend/.env (not backend/law/.env)
backend_dir = Path(__file__).resolve().parent.parent.parent  # backend/law/scripts -> backend
...
def main():
    print("\n" + "=" * 80)
    print("HANG 노드 OpenAI 임베딩 생성 (3072-dim)")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\nProcessing {total_to_process} HANG nodes in batches of {batch_size}...")
        ...
        if total_saved == total_to_process:
            ...
            print(f"\n경고! {total_to_process - total_saved}개 노드 처리 실패")
